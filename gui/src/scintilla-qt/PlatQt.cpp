...
void SurfaceImpl::FillRectangle(PRectangle rc, Fill fill) {
  GetPainter()->fillRect(
    QRectFFromPRect(rc),
    QColorFromColourRGBA(fill.colour));
}
...
void SurfaceImpl::DrawRGBAImage(
  ...
  std::vector<unsigned char> imageBytes =
    ImageByteSwapped(width, height, pixelsImage);
...
void SurfaceImpl::Stadium(PRectangle rc, FillStroke fillStroke, Ends ends) {
  ...
  const Ends leftSide =
    static_cast<Ends>(static_cast<unsigned int>(ends) & 0xfu);
  const Ends rightSide =
    static_cast<Ends>(static_cast<unsigned int>(ends) & 0xf0u);
  switch (leftSide) {
    case Ends::leftFlat:
...
void SurfaceImpl::MeasureWidths(
  ...
  if (mode.codePage == SC_CP_UTF8) {
    ...
    while (ui < fit) {
      ...
      for (size_t bytePos = 0; (bytePos < byteCount) && (i < text.length());
           bytePos++) {
    ...
    for (size_t i = 0; i < text.length();) {
      ...
      for (unsigned int bytePos = 0; (bytePos < lenChar) && (i < text.length());
           bytePos++) {
        positions[i++] = xPosition;
      }
      ui++;
...
void SurfaceImpl::DrawTextNoClipUTF8(
  PRectangle rc,
  const Font *font,
  XYPOSITION ybase,
  std::string_view text,
...
void SurfaceImpl::DrawTextClippedUTF8(
  PRectangle rc,
  const Font *font,
  XYPOSITION ybase,
  std::string_view text,
...
void ListBoxImpl::Create(
  ...
  list->setParent(
    nullptr,
    static_cast<Qt::WindowFlags>(Qt::ToolTip | Qt::FramelessWindowHint));
#endif
...
int ListBoxImpl::Find(const char *prefix) {
  ListWidget *list = GetWidget();
  QString sPrefix =
    unicodeMode ? QString::fromUtf8(prefix) : QString::fromLocal8Bit(prefix);
...
void ListBoxImpl::RegisterImage(int type, const char *xpmData) {
  RegisterQPixmapImage(
    type,
    QPixmap(reinterpret_cast<const char *const *>(xpmData)));
}
...
int ListWidget::currentSelection() {
  ...
  foreach (const QModelIndex ind, indices) { return ind.row(); }
  return -1;
}
...
//   QStyleOptionViewItem result = QListView::viewOptions();
