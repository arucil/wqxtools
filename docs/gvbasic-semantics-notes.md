...
| TAN(`<expr>`) | 正切 |
| VAL(`<expr>`) | 把字符串转换为实数。参数必须是字符串。如果字符串为空或者不是合法的实数，则返回 0。字符串所表示的实数后面可以不是合法的实数部分，只要前面的部分是合法的实数就行（例如 `VAL("13ABC")` 返回 `13`）。 |
