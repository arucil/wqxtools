...
- 变量名可以用空格分隔，但出现在第一个空格之后的部分将被忽略，例如 `AB CDE$` 会被认为和 `AB XY$` 是同一个变量。
